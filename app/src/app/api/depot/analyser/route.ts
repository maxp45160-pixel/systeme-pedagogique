...
export async function GET(request: Request) {
  try {
    ...
    return Response.json(await preparerAnalyseDepot(texteDepot(query.get("documentId"),120),entierDepot(Number(query.get("maximum") ?? 20))), { headers:{"Cache-Control":"no-store"} });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const depot = await analyserDepot(texteDepot(body.documentId,120),texteDepot(body.empreinte,64),entierDepot(body.maximum ?? 20),body.reprise===true,request.signal);
