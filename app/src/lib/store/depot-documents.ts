...
import { comptePiloteDepot, clientInfrastructureDepot } from "./depot-budget";
import { BUCKET_PIECES_JOINTES, estMimePieceJointe, MAX_PIECE_OCTETS } from "@/lib/documents/pieces-jointes";
import { estDepotDocumentaire, MAX_NOTE_DEPOT, type AnalyseDepot, type CouvertureDepot, type DepotDocumentaire, type PageExtraiteDepot, type RestitutionDepot } from "@/lib/documents/depot";
import { objetDepot, texteDepot, validerCouverturesDepot, validerPagesDepot, validerElementsDepot } from "@/lib/documents/depot-validation";
...
function analyseDepuisLigne(value: unknown): AnalyseDepot {
  ...
  if (a.restitution !== null) {
    ...
    if (r.version !== 1) throw new Error("Version de restitution inconnue.");
    restitution = { version: 1, modele: texteDepot(r.modele, 100), creeLe: texteDepot(r.creeLe, 40),
...
export async function lireDepotDocumentaire(id: string): Promise<DepotDocumentaire> {
  ...
  const note = noteDuDepot(document.contenuMd);
  ...
  return { id, titre: document.titre ?? "Dépôt", note, creeLe: document.createdAt ?? "", pieces,
...
export async function lireDepotsDocumentaires(): Promise<{ id: string; titre: string; creeLe: string }[]> {
  const { supabase, userId } = await comptePiloteDepot();
  const { data, error } = await supabase.from("documents").select("id,titre,created_at").eq("user_id",userId)
    .eq("frontmatter->>depot_version","1").order("created_at",{ascending:false}).limit(30);
  if (error) throw new Error("Les dépôts récents ne sont pas disponibles.");
  return (data ?? []).map((r) => ({ id: texteDepot(r.id,120), titre: texteDepot(r.titre,200), creeLe: texteDepot(r.created_at,50) }));
}
...
export async function commencerAnalyseDepot(documentId: string, empreinte: string, reprise: boolean) {
  ...
  const { data, error } = await clientInfrastructureDepot().rpc("depot_demarrer", { p_user_id:userId,p_document_id:documentId,p_empreinte:empreinte,p_reprise:reprise });
...
export async function modifierAnalyseDepot(id: string, tentative: string, valeurs: { pages?: PageExtraiteDepot[]; couvertures?: CouvertureDepot[]; restitution?: RestitutionDepot; statut?: AnalyseDepot["statut"]; erreur?: string; note_source?: string }) {
  const { userId } = await comptePiloteDepot();
  const { data, error } = await clientInfrastructureDepot().from("document_depot_analyses")
    .update({ ...valeurs, updated_at:new Date().toISOString() }).eq("user_id",userId).eq("id",id).eq("tentative",tentative).eq("statut","en-cours").select("id").maybeSingle();
  if (error || !data) throw new Error("L'analyse a été interrompue ou reprise ailleurs.");
}
