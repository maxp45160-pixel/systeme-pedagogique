...
export const VERSION_EXTRACTION_EPUB = "epub-texte-spine-v1";
...
function extraireFichiers(octets: Uint8Array, souhaites: Set<string>, nombre: number): Map<string, string> {
  ...
  const zip = new Unzip((fichier) => {
    ...
    fichier.ondata = (erreur, bloc, fin) => {
      ...
      taille += bloc.length; total += bloc.length;
      if (taille > MAX_XML || total > MAX_DECOMPRESSE) throw new Error("EPUB trop volumineux après décompression.");
      blocs.push(bloc);
      if (fin) {
        const contenu = new Uint8Array(taille);
        let position = 0;
        for (const b of blocs) { contenu.set(b, position); position += b.length; }
        try { fichiers.set(fichier.name, new TextDecoder("utf-8", { fatal: true }).decode(contenu)); }
...
const NON_TEXTUEL = new Set(["img", "svg", "math", "audio", "video", "canvas", "object", "iframe"]);
function texteNoeuds(noeuds: Noeud[], limites: Set<string>, profondeur = 0): string {
  ...
  return noeuds.map((n) => Object.entries(n).map(([nom, valeur]) => {
    if (nom === "#text") return typeof valeur === "string" ? valeur : "";
    ...
    const texte = Array.isArray(valeur) ? texteNoeuds(valeur as Noeud[], limites, profondeur + 1) : "";
    if (nom === "sup" || nom === "sub") {
      limites.add("Exposants et indices transcrits avec leur position ; vérifiez les formules dans l’original.");
      return `${nom === "sup" ? "^" : "_"}{${texte}}`;
    }
    return BLOC.has(nom) ? `\n${texte}\n` : nom === "td" || nom === "th" ? `${texte}\t` : texte;
  }).join("")).join("");
}

/** Un repère par fichier XHTML du spine, sans inventer de pagination. */
export function extraireSectionsEpub(octets: Uint8Array): SectionExtraiteEpub[] {
  const nombre = verifierZip(octets);
  const meta = extraireFichiers(octets, new Set(["mimetype", "META-INF/container.xml"]), nombre);
  ...
  const paquet = xml(extraireFichiers(octets, new Set([opf]), nombre).get(opf)!);
  ...
  const contenus = extraireFichiers(octets, new Set(ordre), nombre);
  return ordre.map((chemin, index) => {
    const racine = xml(contenus.get(chemin)!);
    ...
    const texte = texteNoeuds(enfants(corps[0], "body"), limites).replace(/[ \t]+/g, " ").replace(/ *\n */g, "\n").replace(/\n{3,}/g, "\n\n").trim();
    if (new TextEncoder().encode(texte).length > MAX_TEXTE_SECTION_EPUB) throw new Error("Une section EPUB dépasse 50 000 octets de texte. Exportez un extrait plus court ; aucune partie n’a été analysée.");
    ...
    return { section: { chemin, titre: titre.slice(0, 200), ...(limites.size ? { limites: [...limites] } : {}) }, texte, incertain: limites.size > 0 };
  });
}
