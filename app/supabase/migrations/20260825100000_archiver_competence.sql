-- Migration: 20260825100000_archiver_competence.sql
-- Description : nouvelle commande `archiver_competence` — mettre une compétence
-- de côté sans jamais la supprimer, et rendre la reprise possible.
--
-- Le défaut corrigé : « mettre de côté » passait par `reviser_domaine` avec un
-- retrait, dont l'heuristique SUPPRIME la ligne quand rien ne dépend de la
-- compétence. Or c'est précisément le cas d'une compétence dormante — aucune
-- observation, aucun exercice, aucune relation. L'écran annonçait « rien n'est
-- perdu, vous pouvez la reprendre » ; la ligne était détruite.
--
-- Aucune donnée n'est migrée : ce qui a déjà été supprimé l'est définitivement.
-- La commande ne change rien pour les retraits existants, qui gardent leur
-- heuristique.
...
REVOKE EXECUTE ON FUNCTION public.appliquer_commande_referentiel(TEXT, INTEGER, TEXT, TEXT, JSONB) FROM PUBLIC, anon;
