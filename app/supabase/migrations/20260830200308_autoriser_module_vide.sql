-- Lot 1 — un module académique peut précéder sa première compétence.
--
-- La commande reste SECURITY INVOKER, idempotente et journalisée. Le contrat
-- applicatif n'autorise le tableau vide que quand l'usage demandé est `module`;
-- les autres créations conservent l'invariant métier existant.
...
BEGIN
  ...
  IF v_type = 'creer_domaine' THEN
    ...
    -- Un module académique peut être déclaré avant son premier cours et sa
    -- première compétence. L'application réserve ce tableau vide au parcours
    -- module ; la fonction conserve les mêmes contrôles d'identité et de journal.
