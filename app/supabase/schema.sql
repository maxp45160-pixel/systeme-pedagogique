...
CREATE TABLE IF NOT EXISTS public.sessions (
  ...
  interventions            JSONB,
  -- Lot 3 : provenance compacte d'une acceptation de plan. Le plan complet
  -- reste dérivé et n'est jamais recopié dans cette colonne.
  origine_proposition      JSONB,
  resultat                 TEXT,
  difficulte               TEXT,
...
-- Reçu d'idempotence de l'orchestration. Ce n'est pas une entité de travail :
-- il ne contient qu'une empreinte de commande et son résultat minimal.
...
CREATE TABLE IF NOT EXISTS public.comptes_acces (
  ...
  -- Quota mensuel du tuteur (ADR-116). Posé ici et non dans une table
  -- dédiée : les politiques de `comptes_acces` sont déjà celles qu'un
  -- compteur réclame — lecture soi-ou-admin, écriture administrateur seul.
  -- Un compte lit donc son solde et ne peut pas le remettre à zéro.
  quota_mensuel INTEGER NOT NULL DEFAULT 150 CHECK (quota_mensuel >= 0),
  quota_periode DATE,
  quota_appels INTEGER NOT NULL DEFAULT 0 CHECK (quota_appels >= 0),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
DROP POLICY IF EXISTS orchestration_receipts_read_own ON public.orchestration_command_receipts;
CREATE POLICY orchestration_receipts_read_own
  ON public.orchestration_command_receipts FOR SELECT TO authenticated
  USING ((select auth.uid()) = user_id AND (select public.compte_actif()));
DROP POLICY IF EXISTS orchestration_receipts_insert_own ON public.orchestration_command_receipts;
CREATE POLICY orchestration_receipts_insert_own
  ON public.orchestration_command_receipts FOR INSERT TO authenticated
  WITH CHECK (
    (select auth.uid()) = user_id
    AND (select public.compte_actif())
...
BEGIN
  ...
  SELECT a.user_id, p.email, p.prenom, a.role, a.suspendu_le, a.motif, a.created_at,
    ...
    a.quota_mensuel,
    -- Une période périmée vaut zéro consommé : le compteur repart au premier
    -- appel du mois, l'écran ne doit pas afficher le reliquat du mois
    -- précédent comme s'il courait encore.
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.sessions (
      ...
      v_item->>'plannedFor', v_item->'origineProposition'
    );
  ...
  ) VALUES (v_uid, p_request_id, 'accepter_plan', md5(p_payload::TEXT), v_result);
  RETURN v_result;
END;
$$;
...
-- ADR-143 : pilote documentaire ; migrations appliquées le 06/09/2026.
