...
| Le planificateur et les candidats servent encore aux cours ; la revue et les commandes globales non raccordées ont été retirées le 18/09 | `app/src/lib/engine/planification-temporelle.ts`, `plan-candidates.ts`, ADR-139 | Ne pas confondre ces helpers actifs avec une orchestration globale construite. L'ancienne implémentation de revue/acceptation n'est plus à réactiver. |
