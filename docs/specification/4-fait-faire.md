...
| Workspace focus | — | Aucun mode ou écran dédié | Ne pas dupliquer un geste sans arbitrage | `components/exercices/focus-acte.tsx` (acte ponctuel, pas workspace) | — | ❓ non construit |
| Reporting long terme | Redirections de routes historiques | Pas d'écran dédié ; `/progression` redirige | Ne pas présenter une vue absente comme construite | `app/(app)/progression/page.tsx`, `app/(app)/journal/page.tsx` | — | ❓ |
